...
const Deck3Page = () => {
    ...
    const [originalDeckName, setOriginalDeckName] = useState("Deck3");
    ...
    useEffect(() => {
        const loadDeckFromDB = async () => {
            try {
                ...
                setDeckName(deck.name ?? "Deck3");
                setOriginalDeckName(deck.name ?? "Deck3");
    ...
    const handleSaveDeck = async () => {
        const trimmedDeckName = deckName === '' ? "Deck3" : deckName.slice(0, 15);
        const TARGET_ID = 3;
        const deckData = {
            deckId: TARGET_ID,
